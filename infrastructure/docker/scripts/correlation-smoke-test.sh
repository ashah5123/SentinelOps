...
INCIDENT_RESPONSE="$(curl -sS -X POST "http://127.0.0.1:${INCIDENT_SERVICE_PORT}/api/v1/incidents" \
  ...
  -H "X-Correlation-ID: ${CORRELATION_ID}" \
  -d "{\"title\":\"Smoke test incident ${RUN_ID}\",\"description\":\"Generated by correlation-smoke-test.sh\",\"severity\":\"SEV4\",\"source\":\"smoke-test\",\"affectedService\":\"${TEST_SERVICE}\"}" 2>/dev/null)"
...
if [ -n "${INCIDENT_ID}" ]; then
  ...
  for _ in $(seq 1 15); do
    TIMELINE="$(curl -fsS "http://127.0.0.1:${INCIDENT_SERVICE_PORT}/api/v1/incidents/${INCIDENT_ID}/timeline" 2>/dev/null)"
